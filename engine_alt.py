...
class IndigenousTranslator:
    ...
    def preprocess_text(self, text):
        return " ".join([self.id_stemmer.stem(word) for word in text.lower().split()])
    ...
    def find_closest_match(self, word):
        ...
        stemmed_word = self.preprocess_text(word)
        distances = [(c, lev_distance(stemmed_word, self.preprocess_text(c))) for c in candidates]
        closest = min(distances, key=lambda x: x[1])
        max_len = max(len(stemmed_word), len(closest[0]))
    ...
    def translate_word(self, word):
        ...
        elif stemmed_word in self.dictionary:
            return self.dictionary[stemmed_word], 0.9
        else:
            closest, confidence = self.find_closest_match(word)
            if confidence > 0.6:
                return self.dictionary[closest], confidence
            return word, 0.0
...
if __name__ == "__main__":
    translator = IndigenousTranslator("dictionary.json")
